...
import com.service.Hotels.services.GenericServiceImpl;
...
public class RoomController {
    ...
    @Autowired
    private GenericServiceImpl<Room> service;
    @Autowired
    ...
    public CollectionModel<EntityModel<Room>> getAll(@PathVariable Long id) {
        ...
        List<Room> rooms = service.getAll(id);
        if (rooms.isEmpty()) {
    ...
    public EntityModel<Room> getRoom(@PathVariable Long id) {
        ...
        Room room = service.getEntityById(id);
    ...
    public ResponseEntity<?> addRoom(@RequestBody Room newRoom) {
        EntityModel<Room> roomModel = assembler.toModel(service.addEntity(newRoom));
    ...
    public ResponseEntity<Room> updateRoom(@PathVariable long id, @RequestBody Room newRoom){
        ...
        Room roomAdded = service.addEntity(room);
